...
<div class="page-header d-flex justify-content-between align-items-center">
    <h2><i class="bi bi-plus-circle"></i> Tambah Aset Tetap</h2>
    ...
    <div class="col-md-8">
        <div class="card">
            <div class="card-header">
                <i class="bi bi-building"></i> Form Tambah Aset Tetap
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('aset-tetap.store') }}">
                    @csrf
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="metode_beli"
                                ...
                                <option value="tunai" {{ old('metode_beli') === 'tunai' ? 'selected' : '' }}>Tunai</option>
                                <option value="transfer" {{ old('metode_beli') === 'transfer' ? 'selected' : '' }}>Transfer</option>
                                <option value="kredit" {{ old('metode_beli') === 'kredit' ? 'selected' : '' }}>Kredit</option>
                            </select>
                            @error('metode_beli')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="status"
                                ...
                                @foreach(['Baik','Rusak','Maintenance'] as $s)
                                    <option value="{{ $s }}" {{ old('status', 'Baik') === $s ? 'selected' : '' }}>
                                        {{ $s }}
                                    </option>
                                @endforeach
                            </select>
